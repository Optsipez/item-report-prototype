...
/* STK Age — how long the current stock has been sitting, bucketed per the
   ...
   regardless of which day of either month. __stkAgeSno is the reference
   table's own bucket-order number (0=New … 7=24 Above) — stored only for
   sorting, never displayed.
   Plan Code "N" is its own case, entirely separate from the numeric
   ladder — it's ALWAYS "New" in some form regardless of what the date
   would otherwise compute to: "New*" if Lrcv Date is on file (it just
   hasn't been long enough to matter, or the date's presence itself is
   worth flagging with the asterisk), "New" if there's no date at all.
   Every other Plan Code always runs the real 0–3/3–6/…/24-Above ladder —
   never "New"/"New*", even if the date happens to fall in the current
   month (that no-longer short-circuits to "New" the way it used to; it
   just lands in "0-3" like any other recent date). No Lrcv Date on a
   non-N item is treated as if it were received 01-Jan-2015 — a fixed
   placeholder, not a data patch, so it keeps working once live Navision
   data is connected — and runs through that exact same ladder. */
function stkAgeFor(it){
  const raw = it['Lrcv Date'];
  const m = raw ? String(raw).match(/^(\d{2})-(\d{2})-(\d{4})$/) : null;

  if(String(it['Current Plan Code']).trim().toUpperCase() === 'N'){
    return m ? { label: 'New*', sno: 0 } : { label: 'New', sno: 0 };
  }
...
const COLUMN_LAYOUT = [
  ...
  { type:'core', field:'STK Age', label:'STK Age', sortable:true, stack:true,
    tip:'How long the current stock has been sitting. Plan Code N is always New* (has an Lrcv Date) or New (no Lrcv Date), regardless of age. Every other plan follows 0-3, 3-6, 6-9, 9-12, 12-18, 18-24, or 24 Above (months) from Lrcv Date, or from 01-Jan-2015 if there\'s no Lrcv Date on file.' },
